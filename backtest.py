...
def dv2(close, high, low, length=126):
    # Calculate the DV2 indicator
    dv_raw = ((close / ((high + low) / 2)) - 1).rolling(2).mean()
    return dv_raw.rolling(length).apply(lambda x: pd.Series(x).rank(pct=True).iat[-1]) * 100

def natr(high, low, close):
    # Calculate the NATR indicator
    # 14 days is the recommended smoothing period
    tr = pd.concat([high - low, (high - close.shift()).abs(), (low - close.shift()).abs()], axis=1).max(axis=1)
    return tr.ewm(alpha=1/14).mean() / close * 100

def sma200(close):
    # Simple Moving Average over 200 days
    return close.shift().rolling(200, min_periods=200).mean()

def momentum_6m(close):
    # 6-month momentum (approximately 126 trading days)
...
# Save trades to CSV
trades.to_csv("dv2_trades.csv", index=False)
print("Results saved to dv2_trades.csv")

# Save equity curve to CSV
equity_curve.to_csv("dv2_equity_curve.csv", header=True)
print("Equity curve saved to dv2_equity_curve.csv")
